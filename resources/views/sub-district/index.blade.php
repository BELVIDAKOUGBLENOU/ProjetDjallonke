@extends('layouts.app')
@section('pageTitle', 'Liste des Sub Districts ')

@section('content')

    @php
        /** @var \Illuminate\Pagination\LengthAwarePaginator|\Illuminate\Support\Collection $subDistricts */
        $isPaginator = is_object($subDistricts) && method_exists($subDistricts, 'firstItem');
        $firstItem = $isPaginator ? $subDistricts->firstItem() ?? 1 : 1;
        $totalItems = $isPaginator
            ? $subDistricts->total()
            : ($subDistricts instanceof \Illuminate\Support\Collection
                ? $subDistricts->count()
                : 0);
        $q = request('q');
    @endphp
    ...
    <div>
        <h1 class="text-lg font-semibold tracking-tight theme-title !mt-0">Sub Districts</h1>
        <p class="mb-2 text-sm theme-muted-text">Gérez les différentes Sub Districts de la plateforme.</p>
    </div>
    {{-- Search / Filters --}}
    <div class="theme-surface backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        <form method="GET" action="{{ route('sub-districts.index') }}" class="flex  gap-3 ">
            ...
            {{-- @can('create ' . \App\Models\SubDistrict::getTableName()) --}}

            <a href="{{ route('sub-districts.create') }}"
        ...
        <div class="theme-surface theme-title border border-gray-200/70 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($subDistricts as $subDistrict)
                            <tr class="group hover:bg-indigo-50/40 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $subDistrict->name }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $subDistrict->district?->name }} ({{ $subDistrict->district?->country?->name }})</td>

                                <td class="whitespace-nowrap px-3 py-3 text-right">
                                    <div
                                        class="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition">
                                        {{-- @can('view ' . \App\Models\SubDistrict::getTableName()) --}}

                                        <a href="{{ route('sub-districts.show', $subDistrict->id) }}"
                                            title="{{ __('Show') }}"
                                        ...
                                        <a href="{{ route('sub-districts.edit', $subDistrict->id) }}"
                                            title="{{ __('Edit') }}"
                                        ...
                                        <form action="{{ route('sub-districts.destroy', $subDistrict->id) }}"
            ...
            <div
                ...
                <div class="text-xs text-gray-500 order-2 sm:order-1">
                    @if ($totalItems > 0)
                        @if ($isPaginator)
                            {{ __('Showing') }} {{ $subDistricts->firstItem() }} - {{ $subDistricts->lastItem() }}
                            {{ __('of') }} {{ $subDistricts->total() }}
                        @else
                            {{ __('Total') }} : {{ $totalItems }}
                        @endif
                    @else
