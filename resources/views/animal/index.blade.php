@extends('layouts.app')
@section('pageTitle', 'Liste des Animals ')
...
@section('content')
    ...
    <div x-data="{ importModalOpen: false }">
                            ...
                            <div class="mt-3 text-center sm:mt-0  sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    ...
                                    <ul class="list-disc list-inside text-sm text-gray-600 mb-4 pl-2">
                                        <li>premises_code</li>
                                        <li>species (OVINE, CAPRINE)</li>
                                        <li>sex (M, F)</li>
                                        <li>birth_date (YYYY-MM-DD)</li>
                                        <li>life_status (ALIVE, DEAD, SOLD)</li>
                                    </ul>

                                    <div class="mb-4">
                                        <a href="{{ route('animals.template') }}"
                                    ...
                                    <form action="{{ route('animals.import') }}" method="POST"
        ...
        @php
            /** @var \Illuminate\Pagination\LengthAwarePaginator|\Illuminate\Support\Collection $animals */
            $isPaginator = is_object($animals) && method_exists($animals, 'firstItem');
            $firstItem = $isPaginator ? $animals->firstItem() ?? 1 : 1;
            $totalItems = $isPaginator
                ? $animals->total()
                : ($animals instanceof \Illuminate\Support\Collection
                    ? $animals->count()
                    : 0);
            $q = request('q');
        @endphp
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div>
                <h1 class="text-lg font-semibold tracking-tight theme-title !mt-0">Animals</h1>
                <p class="mb-2 text-sm theme-muted-text">Gérez les différentes Animals de la plateforme.</p>
            </div>
        ...
        <div class="theme-surface backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
            <form method="GET" action="{{ route('animals.index') }}" class="flex  gap-3 ">
                ...
                {{-- @can('create ' . \App\Models\Animal::getTableName()) --}}
            ...
            <div class="theme-surface theme-title border border-gray-200/70 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($animals as $animal)
                                <tr class="group hover:bg-indigo-50/40 transition-colors">
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $animal->uid }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        {{ $animal->created_by }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        {{ $animal->premises_id }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $animal->species }}
                                    </td>
                                    ...
                                    <td class="whitespace-nowrap px-3 py-3 text-right">
                                        <div
                                            ...
                                            {{-- @can('view ' . \App\Models\Animal::getTableName()) --}}
                ...
                <div
                    ...
                    <div class="text-xs text-gray-500 order-2 sm:order-1">
                        @if ($totalItems > 0)
                            @if ($isPaginator)
                                {{ __('Showing') }} {{ $animals->firstItem() }} - {{ $animals->lastItem() }}
                                {{ __('of') }} {{ $animals->total() }}
                            @else
                                {{ __('Total') }} : {{ $totalItems }}
                            @endif
