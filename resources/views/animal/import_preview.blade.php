    ...
    <div x-data="{ activeTab: 'valid' }">
                ...
                <div class="overflow-x-auto mb-4">
                    <table class="min-w-full text-sm bg-white border border-gray-200 rounded-lg">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Premises Code</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Species</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Sex</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Birth Date</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Life Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($validData as $data)
                                <tr>
                                    <td class="px-4 py-2">{{ $data['premises_code'] }}</td>
                                    <td class="px-4 py-2">{{ $data['species'] }}</td>
                                    <td class="px-4 py-2">{{ $data['sex'] }}</td>
                                    <td class="px-4 py-2">{{ $data['birth_date'] }}</td>
                                    <td class="px-4 py-2">{{ $data['life_status'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <form action="{{ route('animals.import.confirm') }}" method="POST">
                    @csrf
                    <button type="submit"
        ...
        <div x-show="activeTab === 'invalid'" class="p-4 rounded-lg bg-gray-50" style="display: none;">
            @if (count($invalidData) > 0)
                <div class="mb-4">
                    <a href="{{ route('animals.import.errors') }}"
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <ul class="list-disc list-inside text-xs text-gray-600">
                                            <li>Premises Code: {{ $row['data']['premises_code'] ?? '' }}</li>
                                            <li>Species: {{ $row['data']['species'] ?? '' }}</li>
                                            <li>Sex: {{ $row['data']['sex'] ?? '' }}</li>
                                            <li>Birth Date: {{ $row['data']['birth_date'] ?? '' }}</li>
                                            <li>Life Status: {{ $row['data']['life_status'] ?? '' }}</li>
                                        </ul>
                                    </td>
