@extends('layouts.app')
@section('pageTitle', 'Liste des Communities ')

@section('content')

    @php
        /** @var \Illuminate\Pagination\LengthAwarePaginator|\Illuminate\Support\Collection $communities */
        $isPaginator = is_object($communities) && method_exists($communities, 'firstItem');
        $firstItem = $isPaginator ? $communities->firstItem() ?? 1 : 1;
        $totalItems = $isPaginator
            ? $communities->total()
            : ($communities instanceof \Illuminate\Support\Collection
                ? $communities->count()
                : 0);
        $q = request('q');
    @endphp
    ...
    <div>
        <h1 class="text-lg font-semibold tracking-tight theme-title !mt-0">Communities</h1>
        <p class="mb-2 text-sm theme-muted-text">Gérez les différentes Communities de la plateforme.</p>
    </div>
    {{-- Search / Filters --}}
    <div class="theme-surface backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        <form method="GET" action="{{ route('communities.index') }}" class="flex  gap-3 ">
        ...
        <div class="theme-surface theme-title border border-gray-200/70 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($communities as $community)
                            <tr class="group hover:bg-indigo-50/40 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $community->name }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $community->creation_date }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $community->creator?->name }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $community->country?->name }}</td>
            ...
            <div
                ...
                <div class="text-xs text-gray-500 order-2 sm:order-1">
                    @if ($totalItems > 0)
                        @if ($isPaginator)
                            {{ __('Showing') }} {{ $communities->firstItem() }} - {{ $communities->lastItem() }}
                            {{ __('of') }} {{ $communities->total() }}
                        @else
                            {{ __('Total') }} : {{ $totalItems }}
                        @endif
                    @else
