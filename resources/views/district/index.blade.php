@extends('layouts.app')
@section('pageTitle', 'Liste des Districts ')

@section('content')

    @php
        /** @var \Illuminate\Pagination\LengthAwarePaginator|\Illuminate\Support\Collection $districts */
        $isPaginator = is_object($districts) && method_exists($districts, 'firstItem');
        $firstItem = $isPaginator ? $districts->firstItem() ?? 1 : 1;
        $totalItems = $isPaginator
            ? $districts->total()
            : ($districts instanceof \Illuminate\Support\Collection
                ? $districts->count()
                : 0);
        $q = request('q');
    @endphp
    ...
    <div>
        <h1 class="text-lg font-semibold tracking-tight theme-title !mt-0">Districts</h1>
        <p class="mb-2 text-sm theme-muted-text">Gérez les différentes Districts de la plateforme.</p>
    </div>
    {{-- Search / Filters --}}
    <div class="theme-surface backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        <form method="GET" action="{{ route('districts.index') }}" class="flex  gap-3 ">
            ...
            {{-- @can('create ' . \App\Models\District::getTableName()) --}}
        ...
        <div class="theme-surface theme-title border border-gray-200/70 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($districts as $district)
                            <tr class="group hover:bg-indigo-50/40 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $district->name }}</td>
                                ...
                                <td class="whitespace-nowrap px-3 py-3 text-right">
                                    <div
                                        ...
                                        {{-- @can('view ' . \App\Models\District::getTableName()) --}}

                                        <a href="{{ route('districts.show', $district->id) }}" title="{{ __('Show') }}"
                                        ...
                                        <a href="{{ route('districts.edit', $district->id) }}" title="{{ __('Edit') }}"
            ...
            <div
                ...
                <div class="text-xs text-gray-500 order-2 sm:order-1">
                    @if ($totalItems > 0)
                        @if ($isPaginator)
                            {{ __('Showing') }} {{ $districts->firstItem() }} - {{ $districts->lastItem() }}
                            {{ __('of') }} {{ $districts->total() }}
                        @else
                            {{ __('Total') }} : {{ $totalItems }}
                        @endif
                    @else
