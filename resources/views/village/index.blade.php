@extends('layouts.app')
@section('pageTitle', 'Liste des Villages ')

@section('content')

    @php
        /** @var \Illuminate\Pagination\LengthAwarePaginator|\Illuminate\Support\Collection $villages */
        $isPaginator = is_object($villages) && method_exists($villages, 'firstItem');
        $firstItem = $isPaginator ? $villages->firstItem() ?? 1 : 1;
        $totalItems = $isPaginator
            ? $villages->total()
            : ($villages instanceof \Illuminate\Support\Collection
                ? $villages->count()
                : 0);
        $q = request('q');
    @endphp
    ...
    <div>
        <h1 class="text-lg font-semibold tracking-tight theme-title !mt-0">Villages</h1>
        <p class="mb-2 text-sm theme-muted-text">Gérez les différentes Villages de la plateforme.</p>
    </div>
    {{-- Search / Filters --}}
    <div class="theme-surface backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-sm mb-4">
        <form method="GET" action="{{ route('villages.index') }}" class="flex  gap-3 ">
            ...
            {{-- @can('create ' . \App\Models\Village::getTableName()) --}}
        ...
        <div class="theme-surface theme-title border border-gray-200/70 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($villages as $village)
                            <tr class="group hover:bg-indigo-50/40 transition-colors">
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $village->name }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $village->local_code }}
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $village->subDistrict?->name }} ({{ $village->subDistrict?->district?->name }} -
                                    {{ $village->subDistrict?->district?->country?->name }})</td>

                                <td class="whitespace-nowrap px-3 py-3 text-right">
                                    <div
                                        class="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition">
                                        {{-- @can('view ' . \App\Models\Village::getTableName()) --}}

                                        <a href="{{ route('villages.show', $village->id) }}" title="{{ __('Show') }}"
                                        ...
                                        <a href="{{ route('villages.edit', $village->id) }}" title="{{ __('Edit') }}"
            ...
            <div
                ...
                <div class="text-xs text-gray-500 order-2 sm:order-1">
                    @if ($totalItems > 0)
                        @if ($isPaginator)
                            {{ __('Showing') }} {{ $villages->firstItem() }} - {{ $villages->lastItem() }}
                            {{ __('of') }} {{ $villages->total() }}
                        @else
                            {{ __('Total') }} : {{ $totalItems }}
                        @endif
                    @else
