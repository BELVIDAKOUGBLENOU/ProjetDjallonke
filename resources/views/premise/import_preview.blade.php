    ...
    <div x-data="{ activeTab: 'valid' }">
                ...
                <div class="overflow-x-auto mb-4">
                    <table class="min-w-full text-sm bg-white border border-gray-200 rounded-lg">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Village ID</th>
                                ...
                                <th class="px-4 py-2 text-left font-medium text-gray-500">GPS Coordinates</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Type</th>
                                <th class="px-4 py-2 text-left font-medium text-gray-500">Health Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($validData as $data)
                                <tr>
                                    <td class="px-4 py-2">{{ $data['village_id'] }}</td>
                                    <td class="px-4 py-2">{{ $data['code'] }}</td>
                                    <td class="px-4 py-2">{{ $data['address'] }}</td>
                                    <td class="px-4 py-2">{{ $data['gps_coordinates'] }}</td>
                                    <td class="px-4 py-2">{{ $data['type'] }}</td>
                                    <td class="px-4 py-2">{{ $data['health_status'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <form action="{{ route('premises.import.confirm') }}" method="POST">
                    @csrf
                    <button type="submit"
        ...
        <div x-show="activeTab === 'invalid'" class="p-4 rounded-lg bg-gray-50" style="display: none;">
            @if (count($invalidData) > 0)
                <div class="mb-4">
                    <a href="{{ route('premises.import.errors') }}"
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <ul class="list-disc list-inside text-xs text-gray-600">
                                            <li>Village ID: {{ $row['data']['village_id'] ?? '' }}</li>
                                            <li>Code: {{ $row['data']['code'] ?? '' }}</li>
                                            <li>Address: {{ $row['data']['address'] ?? '' }}</li>
                                            <li>GPS: {{ $row['data']['gps_coordinates'] ?? '' }}</li>
                                            <li>Type: {{ $row['data']['type'] ?? '' }}</li>
                                            <li>Health: {{ $row['data']['health_status'] ?? '' }}</li>
                                        </ul>
                                    </td>
